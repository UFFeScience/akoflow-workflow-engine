...
func (m *MakeK8sActivityDistributedService) Handle(service MakeK8sJobService) (k8s_job_entity.K8sJob, error) {
	...
	if activitySchedule.ResourceID != "" {
		resource, err := config.App().Repository.ResourceRepository.FindByID(activitySchedule.ResourceID)
		if err != nil {
			return k8s_job_entity.K8sJob{}, err
		}
		if resource != nil {
			// NodeName is Kubernetes provider vocabulary. The domain still uses ResourceID.
			k8sJob.Spec.Template.Spec.NodeName = resource.ProviderID
			return k8sJob, nil
		}
	}

	resourceSelector := m.service.
		makeK8sActivityService.
		SetActivitySchedule(activitySchedule).
		MakeResourceSelector(workflow, activity)

	if resourceSelector != nil {
		k8sJob.Spec.Template.Spec.NodeSelector = resourceSelector
	}
