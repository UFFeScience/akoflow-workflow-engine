...
import (
	...
	"github.com/UFFeScience/akoflow/internal/infrastructure/kubernetes/entity/job"
...
type MakeK8sActivityService struct {
	...
	activitySchedule workflow_activity_entity.ActivitySchedule
}

func (m *MakeK8sActivityService) SetActivitySchedule(activitySchedule workflow_activity_entity.ActivitySchedule) *MakeK8sActivityService {
	m.activitySchedule = activitySchedule
	return m
}
...
func (m *MakeK8sActivityService) addCommandToMonitorFilesStorage(command string, path string) string {
	...
	scriptPath := "/app/internal/infrastructure/assets/scripts/monitor_files_storage.sh"
...
func (m *MakeK8sActivityService) addCommandToMonitorDiskSpecStorage(command string, path string) string {
	port := m.getPortAkoFlowServer()
	scriptPath := "/app/internal/infrastructure/assets/scripts/monitor_disk_spec_storage.sh"
...
// MakeResourceSelector translates a domain resource selection to Kubernetes labels.
func (m *MakeK8sActivityService) MakeResourceSelector(_ workflow_entity.Workflow, wfa workflow_activity_entity.WorkflowActivities) map[string]string {
	resourceSelector := wfa.GetResourceSelector()

	if len(resourceSelector) > 0 || resourceSelector != nil {
		return resourceSelector
	}

	if m.activitySchedule.ResourceID != "" {
		resourceSelector = map[string]string{"akoflow.io/resource-id": m.activitySchedule.ResourceID}
	}
