...
func (m MakeSBatchHPCRuntimeActivityService) GetTemplateSbatch() string {
	...
	if templateSbatchb64 == "" {
		defaultTemplate := utils_read_file.New().ReadFile(fmt.Sprintf("%s/internal/infrastructure/assets/engine_scripts/default-slurm.sbatch", utils_read_file.New().GetRootProjectPath()))
		if defaultTemplate == "" {
