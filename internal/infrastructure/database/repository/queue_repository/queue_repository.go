...
import (
	...
	baserepository "github.com/UFFeScience/akoflow/internal/infrastructure/database/repository"
...
func New() ports.QueueRepository {
	db := (&baserepository.Database{}).Connect()
	if err := schema.Apply(db); err != nil {
		db.Close()
		panic(err)
	}
	return &Repository{db: db, owned: true}
}
