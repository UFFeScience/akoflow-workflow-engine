...
func seedPlanParents(t *testing.T, repository *Repository) {
	...
	statements := []string{
		...
		`INSERT INTO activity_definitions(id, workflow_version_id, activity_type_id, external_id, name, kind, capabilities, command_spec, resource_requirements, policy) VALUES ('task', 'w1', 'type', 'task', 'task', 'task', '{}', '{}', '{}', '{}')`,
...
func TestSchedulePlanSaveErrors(t *testing.T) {
	repository := setup(t)
	bad := domain.SchedulePlan{ID: "bad", WorkflowVersionID: "w1", EnvironmentVersionID: "e1", Source: domain.PlanningSourcePlugin, Assignments: []domain.PlanAssignment{{ID: "a", ActivityID: "task", ResourceID: "r1", Metadata: map[string]any{"channel": make(chan int)}}}}
