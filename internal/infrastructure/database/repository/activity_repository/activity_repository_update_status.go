...
func (w *ActivityRepository) UpdateResourceSelector(id int, resourceSelector string) error {
	database := repository.Database{}
	c := database.Connect()

	_, err := c.Exec("UPDATE "+w.tableNameActivity+" SET resource_selector = ? WHERE ID = ?", resourceSelector, id)
	if err != nil {
		return err
	}
