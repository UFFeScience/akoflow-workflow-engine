...
func (w *ActivityRepository) Find(id int) (workflow_activity_entity.WorkflowActivities, error) {
	...
	rows, err := c.Query("SELECT id, workflow_id, namespace, name, image, runtime, resource_k8s_base64, status, proc_id, resource_selector, mount_path, created_at, started_at, finished_at FROM "+w.tableNameActivity+" WHERE id = ?", id)
	if err != nil {
