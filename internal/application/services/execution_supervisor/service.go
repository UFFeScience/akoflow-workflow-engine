...
func New(executions ports.ExecutionRepository, activities ActivityController, simulation ports.PlanExecutor, config Config) (*Service, error) {
	...
	return &Service{executions: executions, activities: activities, simulation: simulation, config: config}, nil
}

func (s *Service) Execute(ctx context.Context, request ports.ExecutionRequest) (trace domain.ExecutionTrace, err error) {
...
func (s *Service) executeActivities(ctx context.Context, request ports.ExecutionRequest) (domain.ExecutionTrace, error) {
	...
	for len(completed) < len(activities) {
		...
		for activityID, handle := range running {
			observed, err := s.activities.Inspect(ctx, handle.ID, request.Run.Mode)
			if err != nil {
				return domain.ExecutionTrace{}, fmt.Errorf("inspect activity %q: %w", activityID, err)
			}
			task := tasks[activityID]
			switch observed.Status {
			case domain.HandleCompleted:
				task.Status, task.FinishedAt = domain.TaskCompleted, observed.FinishedAt
				task.RuntimeSeconds = maxFloat(0, observed.FinishedAt-observed.StartedAt)
				if err := s.executions.SaveTask(ctx, task); err != nil {
					return domain.ExecutionTrace{}, err
				}
				completed[activityID] = task
			...
			case domain.HandleFailed, domain.HandleStopped:
				...
				return domain.ExecutionTrace{}, fmt.Errorf("activity %q failed: %s", activityID, observed.Failure)
			}
		}
		...
		for _, activityID := range ready[:available] {
			activity, assignment := activities[activityID], assignments[activityID]
			resource, ok := resources[assignment.ResourceID]
			if !ok {
				return domain.ExecutionTrace{}, fmt.Errorf("resource %q not found", assignment.ResourceID)
			}
			handle, err := s.activities.Start(ctx, domain.ActivityExecutionContext{Run: request.Run,
				Workflow: request.Workflow, Activity: activity, Assignment: assignment, Resource: resource})
			if err != nil {
				return domain.ExecutionTrace{}, fmt.Errorf("start activity %q: %w", activityID, err)
			}
			task := domain.TaskExecution{ID: request.Run.ID + ":" + activityID, ExecutionRunID: request.Run.ID,
			...
			if handle.Status == domain.HandleCompleted {
				...
				task.FinishedAt = handle.FinishedAt
