...
func (s *Connection) AddHost(host ssh_client_entity.SSHClient) {
...
func (s *Connection) connect(client ssh_client_entity.SSHClient) (*ssh.Client, error) {
...
func (s *Connection) addConnection(connection *ssh.Client) {
	s.connections = append(s.connections, connection)
}

func (s *Connection) EstablishConnectionWithHosts() {
...
func (s *Connection) GetMainNode() ssh_client_entity.SSHClient {
	return s.hosts[0]
}

func (s *Connection) GetAllHosts() []ssh_client_entity.SSHClient {
	return s.hosts
}

func (s *Connection) GetWorkerNodes() []ssh_client_entity.SSHClient {
	if len(s.hosts) <= 1 {
...
func (s *Connection) establishConnection(client ssh_client_entity.SSHClient, results chan<- string) {
...
func (s *Connection) CloseConnections() {
	for _, connection := range s.connections {
		connection.Close()
	}
}

func (s *Connection) ExecuteCommandsInMultipleHost(commands []string) {
	var wg sync.WaitGroup
...
func (s *Connection) ExecuteCommandsOnHost(client ssh_client_entity.SSHClient, commands []string) string {
	connection, err := s.connect(client)
	response := ""
	if err != nil {
